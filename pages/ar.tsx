...
export const getServerSideProps: GetServerSideProps = async () => {
  ...
  const banks = (
    await prisma.bank.findMany({ include: { aprValues: true, saleReps: true } })
  ).map((bank) => {
...
class Ar extends React.Component<Props, State> {
  ...
  componentDidMount() {
    ...
    this.getSalaryGroup(this.state.salary).then((salaryGroup) =>
      this.setState({ salaryGroup })
    );
  }
  ...
  async search(e) {
    e?.preventDefault();
    const { loan, salary, salaryGroup, useMaxLoan, sector, duration } =
      this.state;
    ...
    const minSalary =
      banks[0].minSalary < salaryGroups[0].min
        ? salaryGroups[0].min
        : banks[0].minSalary;
    ...
    const highestMaxLoan =
      (salary * (1 / 3) * (duration * 12)) /
      (1 + (lowestApr?.value / 100) * duration);

    if (
      loan < minLoan ||
      salary < minSalary ||
      (loan > highestMaxLoan && !useMaxLoan)
    ) {
      const errors = [];

      if (loan > highestMaxLoan)
        alert(
          `عذرا, بناء على الراتب والمدة فان الحد الاعلى للتمويل هو ${highestMaxLoan.toFixed(
            0
          )}`
        );
      if (loan < minLoan && !useMaxLoan)
        alert(
          ` عذرا ، الحد الأدنى لمبلغ التمويل الذي يمكننا العثور عليه هو ${minLoan}`
        );
      if (salary < minSalary)
        alert(`عذرا ، لم يتم العثور على أي بنك بالراتب الذي تم إدخاله`);
    ...
    const banksProps: BankProps[] = banks
      .map((bank) => {
        ...
        bankProps.total = useMaxLoan
          ? salary * (1 / 3) * (duration * 12)
          : loan + loan * duration * (bankProps.apr / 100);
        bankProps.loan = useMaxLoan
          ? bankProps.total / (1 + (bankProps.apr / 100) * duration)
          : loan;
  ...
  render() {
    ...
    const banks = [...banksProps]
      .sort((a, b) => (filter?.sort ? filter.sort(a, b) : 0))
      .filter((bank) => (filter?.filter ? filter.filter(bank) : true));

    const sectors = this.props.sectors.map((sector, index) => (
      <li
        key={index}
        className={
          this.state.sector?.name === sector.nameAR ? styles.active : ""
        }
        onClick={() => this.setState({ sector, showSector: false })}
      >
        {sector.nameAR}
      </li>
    ));
    ...
    const filters = Filters.map((filter, index) => (
      <li
        key={index}
        className={
          this.state.filter?.nameAR === filter.nameAR ? styles.active : ""
        }
        onClick={() => this.setState({ filter, showFilter: false })}
      >
        {filter.nameAR}
      </li>
    ));
    ...
    return (
      <div className={styles.container}>
        <Head>
          ...
          <meta
            name="description"
            content="احصل على اقل نسبة ارباح للتمويل الشخصي او العقاري من جميع البنوك في السعودية"
          />
        ...
        <section className={styles.search}>
          <div className={styles.wrapper}>
            <h5 className={styles.mainTitle}>
              قارن بين أفضل عروض التمويل الشخصي
            </h5>
              ...
              <div className={styles.row}>
                <div className={styles.inputContainer}>
                  ...
                  <input
                    ...
                    onChange={(e) => {
                      const loan = this.parseNumber(
                        this.parseNumber(e.target.value) > 1000000
                          ? "1000000"
                          : e.target.value
                      );
                      this.setState({ loan });
                ...
                <div className={styles.inputContainer}>
                  ...
                  <input
                    ...
                    onChange={(e) => {
                      const salary = this.parseNumber(
                        this.parseNumber(e.target.value) > 90000
                          ? "90000"
                          : e.target.value
                      );
                      this.getSalaryGroup(salary).then((salaryGroup) =>
                        this.setState({ salaryGroup })
                      );
                ...
                <div className={styles.checkBoxContainer}>
                  <button
                    ...
                    id="useMaxLoan"
                    onClick={() =>
                      this.setState({ useMaxLoan: !this.state.useMaxLoan })
                    }
                  >
                ...
                <div className={styles.selectContainer}>
                  ...
                  >
                    ...
                    <input
                      ...
                      type="text"
                      readOnly
                      value={this.state.sector?.nameAR}
                    />
                  </div>

                  <ul
                    className={styles.menu}
                    style={this.state.showSector ? { display: "block" } : {}}
                  >
                    {sectors}
                  </ul>
                </div>
          ...
          <div className={styles.wrapper}>
            <div className={styles.filter}>
              ...
              {this.state.showFilter && (
                <ul className={styles.menu}>{filters}</ul>
              )}
            </div>
