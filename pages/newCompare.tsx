...
export async function getServerSideProps() {
  try {
    ...
    console.log("job sectors", jobSectors.length);
...
function Compare(props: Props) {
  ...
  }, [duration, jobSector, loanAmount, monthlySalary, search, shouldSearch, useMaxLoan]);
  ...
  const [expenseAutoFill, setExpenseAutoFill] = React.useState({
    bill: 0,
    rent: 0,
    food: 0,
    car_insurance: 0,
  ...
  async function search(
    ...
    const sector =
      props.jobSectors &&
      props.jobSectors.find((obj) => obj.name === processedJobSector);
    ...
    if (processedLoanAmount > maximumLoan && !processedUseMaxLoan)
      setErrorString(
        `عذرا، بناء على الراتب والمدة فان الحد الاعلى للتمويل هو ${maximumLoan.toLocaleString(
          undefined
        )} ر.س`
      );
    if (processedLoanAmount < minLoan && !processedUseMaxLoan)
      setErrorString(
        `عذرا ، الحد الأدنى لمبلغ التمويل الذي يمكننا العثور عليه هو ${minLoan.toLocaleString(
          undefined
        )} ر.س`
      );
    if (processedMonthlySalary < minSalary)
      setErrorString("عذرا ، لم يتم العثور على أي بنك بالراتب الذي تم إدخاله");
                ...
                return (
                  ...
                  >
                    ...
                    <NewBankCardOldDesign
                      ...
                      setExpenseAutoFill={setExpenseAutoFill}
                    />
                  </div>
                );
              })}
